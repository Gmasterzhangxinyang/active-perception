...
class VisionLLM:
    ...
    def analyze_single_image(self, image_tensor, camera_id):
        ...
        Returns:
            dict: 分析结果
        """
        prompt = f"""你是一个专业的自动驾驶相机图像质量评估专家。请分析这张 {self.camera_names.get(camera_id, f"Camera{camera_id}")} 图片，判断天气、光照、模糊和局部问题。

只返回JSON对象，不要返回Markdown。格式如下:
{{
  "camera_id": {camera_id},
  "camera_name": "{self.camera_names.get(camera_id, f"Camera{camera_id}")}",
  "analysis": "简要描述图像状况",
  "conditions": ["rain", "fog", "haze", "clear", "low_light", "glare", "overexposed", "blur", "motion_blur", "noise"],
  "problem_regions": [
    {{"bbox": [180, 40, 320, 100], "condition": "rain", "severity": "medium"}}
  ],
  "suggested_tools": [
    {{"tool": "remove_rain", "target_regions": [[180, 40, 320, 100]], "reason": "有明显雨滴"}},
    {{"tool": "enhance_low_light", "target_regions": [], "reason": "低照度"}},
    {{"tool": "reduce_glare", "target_regions": [], "reason": "强车灯眩光"}},
    {{"tool": "deblur_image", "target_regions": [], "reason": "运动模糊"}}
  ]
}}
  ...
  "suggested_tools": []
    ...
    def merge_analyses(self, analyses):
        ...
        tool_plan = {
            ...
            "enhance_low_light": {"camera_ids": [], "regions": []},
            "reduce_glare": {"camera_ids": [], "regions": []},
            "sharpen_image": {"camera_ids": [], "regions": []},
            "deblur_image": {"camera_ids": [], "regions": []},
        ...
        for analysis in analyses:
            ...
            for tool_info in tools:
                ...
                elif tool_name in {"enhance_low_light", "reduce_glare", "sharpen_image", "deblur_image"}:
                    if cam_id not in tool_plan[tool_name]["camera_ids"]:
                        tool_plan[tool_name]["camera_ids"].append(cam_id)
                    tool_plan[tool_name]["regions"].extend(regions)
